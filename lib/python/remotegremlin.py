...
import rs274.remoteglcanon as glcanon
import rs274.remoteinterpret as interpret
import linuxcnc
import gcode
...
class Gremlin(gtk.gtkgl.widget.DrawingArea, glnav.GlNavBase,
    ...
    def __init__(self, inifile, stat):
        ...
        for i,j in enumerate("XYZABCUVW"):
            if self.stat.config.axis_mask & (1<<i) == 0: continue
            live_axis_count += 1
    ...
    def poll(self):
        if not self.stat.synced:
            return

        s = self.stat
        fingerprint = (self.logger.npts, self.soft_limits(),
            s.motion.actual_position, s.motion.joint_actual_position,
            s.motion.homed, s.motion.g5x_offset, s.motion.g92_offset, s.motion.limit, s.io.tool_in_spindle,
            s.motion.motion_mode, s.motion.current_vel)
    ...
    def load(self, filename = None):
        status = self.stat
        if not filename and status.task.file:
            filename = status.task.file
        elif not filename and not status.task.file:
            return
        ...
        try:
            ...
            canon = StatCanon(self.colors, self.get_geometry(), self.lathe_option, status, random)
            ...
            unitcode = "G%d" % (20 + (status.config.linear_units == application.CANON_UNITS_MM))
    ...
    def get_current_tool(self):
        for i in self.stat.io.tool_table:
            if i[0] == self.stat.io.tool_in_spindle:
                return i
