...
class GlCanonDraw:
    ...
    def to_internal_linear_unit(self, v, unit=None):
        if unit is None:
            unit = self.stat.config.linear_units
        lu = (unit or 1) * 25.4
        return v/lu
    ...
    def draw_grid(self):
        ...
        view = self.get_view()
        if view == p: return
        rotation = math.radians(self.stat.motion.rotation_xy % 90)
    ...
    def posstrs(self):
        s = self.stat
        limit = []
        for i,l in enumerate(s.motion.limit):
            if s.config.axis_mask & (1<<i):
                limit.append(l)

        homed = []
        for i,h in enumerate(s.motion.homed):
            if s.config.axis_mask & (1<<i):
                homed.append(h)

        if self.is_lathe() and not s.config.axis_mask & 2:
            homed.insert(1, 0)
            limit.insert(1, 0)

        if not self.get_joints_mode():
            if self.get_show_commanded():
                positions = s.motion.position
            else:
                positions = s.motion.actual_position

            if self.get_show_relative():
                positions = [(i-j) for i, j in zip(positions, s.io.tool_offset)]
                positions = [(i-j) for i, j in zip(positions, s.motion.g5x_offset)]

                t = -s.motion.rotation_xy
                t = math.radians(t)
                x = positions[0]
                y = positions[1]
                positions[0] = x * math.cos(t) - y * math.sin(t)
                positions[1] = x * math.sin(t) + y * math.cos(t)
                positions = [(i-j) for i, j in zip(positions, s.motion.g92_offset)]
            ...
            axisdtg = self.to_internal_units(s.motion.dtg)
            g5x_offset = self.to_internal_units(s.motion.g5x_offset)
            g92_offset = self.to_internal_units(s.motion.g92_offset)
            tlo_offset = self.to_internal_units(s.io.tool_offset)
            dtg = self.to_internal_linear_unit(s.motion.distance_to_go)
            spd = self.to_internal_linear_unit(s.motion.current_vel)
            ...
            posstrs = ["  %s:% 9.4f" % i for i in
                zip(range(self.get_num_joints()), s.motion.joint_actual_position)]
            droposstrs = posstrs
    ...
    def dro_format(self,s,spd,dtg,limit,homed,positions,axisdtg,g5x_offset,g92_offset,tlo_offset):
            ...
            droposstrs.append(rotformat % (label, 'R', s.motion.rotation_xy))
